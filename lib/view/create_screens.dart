...
class RenderBlendMask extends RenderProxyBox {
  ...
  void paint(PaintingContext context, Offset offset) {
    // When blend mode is normal (srcOver), skip saveLayer for performance.
    if (_blendMode == BlendMode.srcOver && _opacity == 1.0) {
      super.paint(context, offset);
      return;
    }

    // saveLayer composites the child subtree onto an offscreen buffer,
    // then draws that buffer onto the parent using the chosen blend mode.
...
// Empty State Icon (Moved here so it's defined before use)
        ...
        if (p.showLayerPanel)
          Positioned(
            ...
            child: LayerPanel(
              ...
              onToggleVisibility: p.toggleLayerVisibility,
              onDelete: p.deleteLayer,
              onRename: p.renameLayer,
              onClose: p.closeAllPanels,
              onBringToFront: p.bringToFront,
              onSendToBack: p.sendToBack,
              onBringForward: p.bringForward,
              onSendBackward: p.sendBackward,
              onDeleteItem: p.deleteItem,
              onReorder: (items) {
                p.canvasItems
                  ..clear()
                  ..addAll(items);
...
class _DraggableStyledTextState extends State<_DraggableStyledText> {
  ...
  void _editText() async {
    final ctrl = TextEditingController(text: widget.style.text);
        ...
        onScaleUpdate: (details) {
          setState(() {
            if (details.pointerCount == 1) {
              ...
              // Scale (pinch to resize text)
              _fontSize = (_fontSize * details.scale).clamp(8.0, 200.0);
            }
          });
        },
        child: Column(
